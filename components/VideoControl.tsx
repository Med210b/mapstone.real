...
const VideoControl: React.FC = () => {
  const { toggleVideo, isVideoEnabled } = useVideo();
  ...
  return (
    <motion.button
      ...
      title={isVideoEnabled ? "Disable Background Video" : "Enable Background Video"}
    >
      <span className="text-xs font-subtitle uppercase tracking-widest hidden md:block">
        {isVideoEnabled ? 'Live View' : 'Static View'}
      </span>
      <div className="relative">
        {isVideoEnabled ? <Pause size={16} /> : <Play size={16} />}
      </div>
    </motion.button>
  );
};
