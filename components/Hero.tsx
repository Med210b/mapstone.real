...
const Hero: React.FC = () => {
  ...
  // Parallax effect for the background image to add depth on scroll
  ...
  return (
    <section id="home" className="relative h-[100dvh] w-full overflow-hidden flex items-center justify-center bg-black p-0">
      ...
      {/* Background Image Layer - Optimized for Phone and Desktop */}
      <motion.div 
        style={{ y: yBg }}
      ...
      >
        <div 
          ...
          style={{ 
            backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,0.3), rgba(0,0,0,0.7)), url(${HERO_IMAGE})`,
          }} 
        />
      </motion.div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-10 flex flex-col items-center gap-2">
        <span className="text-[10px] text-[#D4AF37] font-subtitle tracking-[0.3em] uppercase opacity-60">
            {t.hero.explore}
        </span>
