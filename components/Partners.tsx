...
import { Globe, Award, ShieldCheck } from 'lucide-react'; // Icons for the cards
...
// --- 2. 3D CARD COMPONENTS (Fixed with React.FC to prevent errors) ---
...
const CardItem: React.FC<{ 
  ...
  [key: string]: any; 
}> = ({ as: Tag = "div", children, className, translateX = 0, translateY = 0, translateZ = 0, ...rest }) => {
...
const pillars = [
  {
    title: "Global Reach",
    desc: "Connecting Dubai's finest developments to investors in London, New York, and Singapore.",
    icon: <Globe className="w-10 h-10 text-[#D4AF37]" />
  },
  {
    title: "Elite Standards",
    desc: "We only partner with developers who meet our rigorous criteria for quality and delivery.",
    icon: <Award className="w-10 h-10 text-[#D4AF37]" />
  },
  {
    title: "Secure Investment",
    desc: "Ensuring every project offers not just luxury, but long-term capital appreciation.",
...
const Partners: React.FC = () => {
  const { t } = useLanguage();
  // Deep Red color matching your uploaded image
  const deepRed = "#8B2020";
  ...
  return (
    <section className="min-h-screen bg-black py-12 px-4 sm:py-16 sm:px-6 lg:px-8 border-b border-[#d49842]/30 overflow-hidden relative">
      <BackgroundEffect color={deepRed} opacity={0.15} />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {/* NEW SECTION: 3D TEXT CARDS (Red Theme) */}
        <div className="mt-20 md:mt-32">
          <div className="text-center mb-12">
            <h3 className="text-[#D4AF37] font-subtitle tracking-widest uppercase text-sm mb-2">Our Standards</h3>
            <h2 className="text-3xl md:text-4xl text-white font-header">Why We Partner</h2>
          </div>
              ...
              <CardContainer key={i} className="inter-var w-full">
                <CardBody className="bg-black/80 relative group/card border border-[#8B2020]/50 w-full h-auto rounded-xl p-8 hover:shadow-2xl hover:shadow-[#8B2020]/40 transition-all duration-500">
                  ...
                  <CardItem translateZ="50" className="w-full mb-6">
                    <div className="w-16 h-16 rounded-full bg-[#8B2020]/20 border border-[#8B2020] flex items-center justify-center group-hover/card:scale-110 transition-transform duration-300 shadow-[0_0_15px_rgba(139,32,32,0.5)]">
                      {pillar.icon}
                    </div>
                  </CardItem>

                  <CardItem translateZ="40" className="text-xl font-bold text-white mb-3 group-hover/card:text-[#D4AF37] transition-colors">
                    {pillar.title}
                  </CardItem>

                  <CardItem translateZ="30" className="text-gray-400 text-sm leading-relaxed">
                    {pillar.desc}
                  </CardItem>
                </CardBody>
              </CardContainer>
