    ...
    <section className="min-h-screen bg-black py-12 px-4 sm:py-16 sm:px-6 lg:px-8 border-b border-[#d49842]/30 overflow-hidden relative">
      <style>{`
        @keyframes orbit3d {
          ...
          100% { transform: rotateY(360deg) translateZ(450px) rotateY(-360deg); }
        }
        @keyframes orbit3d-mobile {
          0% { transform: rotateY(0deg) translateZ(250px) rotateY(0deg); }
          100% { transform: rotateY(360deg) translateZ(250px) rotateY(-360deg); }
        }
        .carousel-3d {
          position: relative;
          width: 100%;
        ...
        @media (min-width: 768px) {
          .carousel-item {
            animation: orbit3d 24s linear infinite;
            animation-delay: calc(var(--index) * -2.4s);
          }
        }
        .carousel-card {
          ...
          border: 1px solid rgba(212, 152, 66, 0.2);
          border-radius: 12px;
          padding: 16px;
          display: flex;
          align-items: center;
        ...
        .carousel-item:hover .carousel-card {
          border-color: rgba(212, 152, 66, 0.8);
          box-shadow: 0 0 30px rgba(212, 152, 66, 0.3);
            ...
            {partners.map((partner, index) => (
              <div key={index} className="carousel-item" style={{ "--index": index, "--total": partners.length } as React.CSSProperties}>
                <div className="carousel-card group">
                  <img src={partner.logo} alt={partner.name} className="max-w-full max-h-full object-contain filter brightness-0 invert opacity-70 group-hover:opacity-100 transition-opacity duration-300" />
                </div>
              </div>
            ))}
          </div>
